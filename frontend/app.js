...
// ─── Presets de Classe de Ativo ───────────────────────────────────────────────

const PRESETS = {
  cdb: { ir_pf: 15, isento_pf: false, spread_pj: 0, lockIr: false },
  lci_lca: { ir_pf: 0, isento_pf: true, spread_pj: 0, lockIr: true },
  cri_cra: { ir_pf: 0, isento_pf: true, spread_pj: 0, lockIr: true },
  titulo_publico: {
    ir_pf: 15,
    isento_pf: false,
    spread_pj: 0.5,
  ...
  personalizado: { ir_pf: 15, isento_pf: false, spread_pj: 0, lockIr: false },
};
...
function applyPreset(key) {
  ...
  const irSlider = document.getElementById("ir_pf_rate");
  const irLabel = document.getElementById("lbl-irpf");
  const isentoCheck = document.getElementById("isento_pf");
  const spreadSlider = document.getElementById("spread_pj");
  const spreadLabel = document.getElementById("lbl-spread");

  // IR PF — não usar disabled (FormData exclui inputs disabled)
  irSlider.value = p.lockIr ? 15 : p.ir_pf;
  irLabel.textContent = p.isento_pf ? "Isento" : p.ir_pf + "%";
  irSlider.closest(".field").classList.toggle("field-locked", p.lockIr);

  // Isenção
  isentoCheck.checked = p.isento_pf;

  // Spread
  spreadSlider.value = p.spread_pj;
  spreadLabel.textContent = p.spread_pj + "%";
}
  ...
  anos.forEach((a) => {
    ...
    if (includeCustos) {
      tr.innerHTML = `
        ...
        <td class="td-negative">${fmt(a.custos_ibs_cbs)}</td>
...
function renderResults(data) {
  ...
  document.getElementById("card-label-pj").textContent =
    `VFL · ${data.nome_pj}`;
...
function renderItcmd(data) {
  ...
  document.getElementById("itcmd-aliq-pf").textContent = PCT(
    d.aliquota_efetiva_pf,
  );
  document.getElementById("itcmd-vfl-suc-pf").textContent = fmt(
    d.vfl_pos_sucessao_pf,
  );
  ...
  document.getElementById("itcmd-aliq-pj").textContent = PCT(
    d.aliquota_efetiva_pj,
  );
  document.getElementById("itcmd-vfl-suc-pj").textContent = fmt(
    d.vfl_pos_sucessao_pj,
  );
  ...
  econEl.className =
    "itcmd-economia-valor" + (d.economia < 0 ? " negativo" : "");

  const pct =
    d.itcmd_pf > 0 ? ((d.economia / d.itcmd_pf) * 100).toFixed(1) : "0.0";
  document.getElementById("itcmd-economia-sub").textContent =
    d.economia >= 0
...
function switchChart(btn) {
  document
    .querySelectorAll(".chart-card .tab-btn")
...
function switchTable(scenario, btn) {
  document
    .querySelectorAll(".table-card .tab-btn")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
...
document.getElementById("calc-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const payload = {
      ...
      desconto_quota_holding:
        parseFloat(fd.get("desconto_quota_holding")) / 100,
      isento_pf: fd.get("isento_pf") === "on",
      spread_pj: parseFloat(fd.get("spread_pj")) / 100,
      ibs_cbs_rate: parseFloat(fd.get("ibs_cbs_rate")) / 100,
    };
    ...
    if (!res.ok) {
      ...
      throw new Error(msg || "Erro no servidor");
    }
...
// ─── Classe de Ativo: preset switcher ─────────────────────────────────────────

document.getElementById("classe_ativo").addEventListener("change", (e) => {
